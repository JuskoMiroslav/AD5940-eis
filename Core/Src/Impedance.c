 ...
 */
AppIMPCfg_Type AppIMPCfg = { .bParaChanged = bFALSE,
		.SeqStartAddr = 0,
		.MaxSeqLen = 0,
		...
		.LptiaRtiaSel = LPTIARTIA_200R, /* COnfigure RTIA */
...
AD5940Err AppIMPCtrl(uint32_t Command, void *pPara) {
	...
	case IMPCTRL_START: {
		...
		wupt_cfg.SeqxWakeupTime[SEQID_0] =
				(uint32_t) (AppIMPCfg.WuptClkFreq
						/ AppIMPCfg.ImpODR) - 4;
	...
	case IMPCTRL_SHUTDOWN: {
		AppIMPCtrl(
		IMPCTRL_STOPNOW, 0); /* Stop the measurement if it's running. */
...
static AD5940Err AppIMPSeqMeasureGen(void) {
	...
	clks_cal.RatioSys2AdcClk = AppIMPCfg.SysClkFreq
			/ AppIMPCfg.AdcClkFreq;
	...
	AppIMPSetCalibrationResistor(AppIMPCfg.RcalSelection);	
	/* Disconnect SE0 from LPTIA*/
	LpAmpCfg.LpAmpPwrMod =
	LPAMPPWR_NORM;
 ...
 AFECTRL_SINC2NOTCH, bTRUE);
	...
	AD5940_AFECtrlS(
	AFECTRL_ADCPWR | AFECTRL_SINC2NOTCH, bTRUE); /* Enable Waveform generator */
	//delay for signal settling DFT_WAIT
	AD5940_SEQGenInsert(SEQ_WAIT(16 * 10));
	AD5940_AFECtrlS(
	AFECTRL_ADCCNV | AFECTRL_DFT, bTRUE); /* Start ADC convert and DFT */
 ...
 AFECTRL_WG
					| AFECTRL_DACREFPWR | AFECTRL_HSDACPWR
					|\
 AFECTRL_SINC2NOTCH | AFECTRL_DFT
					| AFECTRL_ADCCNV, bFALSE);
 ...
 AFECTRL_WG
	...
	AD5940_SEQGenInsert(SEQ_WAIT(16 * 10)); //delay for signal settling DFT_WAIT
	AD5940_AFECtrlS(
	AFECTRL_ADCCNV | AFECTRL_DFT/*|AFECTRL_SINC2NOTCH*/,
			bTRUE); /* Start ADC convert and DFT */
	AD5940_SEQGenInsert(SEQ_WAIT(WaitClks)); /* wait for first data ready */
	AD5940_AFECtrlS(
			AFECTRL_ADCCNV | AFECTRL_DFT | AFECTRL_WG
...
AD5940Err AppIMPInit(uint32_t *pBuffer, uint32_t BufferSize) {
	...
	/* Reconfigure FIFO */
	AD5940_FIFOCtrlS(
	FIFOSRC_DFT, bFALSE); /* Disable FIFO firstly */
	...
	while (AD5940_INTCTestFlag(
	AFEINTC_1, AFEINTSRC_ENDSEQ) == bFALSE)
		;
	...
	AD5940_AFEPwrBW(AppIMPCfg.PwrMod,
	AFEBW_250KHZ);

	AD5940_WriteReg(
	REG_AFE_LPTIASW0, 0x3180);
...
int32_t AppIMPRegModify(int32_t *const pData, uint32_t *pDataCount) {
	...
	if (AppIMPCfg.SweepCfg.SweepEn) /* Need to set new frequency and set power mode */
	{
		...
		AD5940_WGFreqCtrlS(AppIMPCfg.SweepNextFreq,
...
int32_t AppIMPDataProcess(int32_t *const pData, uint32_t *pDataCount) {
	...
	*pDataCount = 0;
	...
	for (uint32_t i = 0; i < ImpResCount; i++) {
		if (1) {
			fImpCar_Type DftRcal, DftRzRload, DftRload,
					temp1, temp2, res;
			...
			//temp1 = 1/DftRzRload;
			//temp2 = 1/DftRload;
			temp1 = AD5940_ComplexDivFloat(&DftConst1,
					&DftRzRload);
			temp2 = AD5940_ComplexDivFloat(&DftConst1,
					&DftRload);
			res = AD5940_ComplexSubFloat(&temp1, &temp2);
			res = AD5940_ComplexMulFloat(&res, &DftRcal);
			pOut[i].Magnitude = AD5940_ComplexMag(&res)
					* AppIMPCfg.RcalVal;
//			pOut[i].Magnitude = AD5940_ComplexMag(&res)
//								* AppIMPCfg.RtiaCalTable[i][0];

			pOut[i].Phase = AD5940_ComplexPhase(&res);
		} else {
			iImpCar_Type *pDftRcal, *pDftRzRload,
					*pDftRload;
			...
			RzRloadMag = sqrt(
					(float) pDftRzRload->Real
							* pDftRzRload->Real
							+ (float) pDftRzRload->Image
									* pDftRzRload->Image);
			RzRloadPhase = atan2(-pDftRzRload->Image,
					pDftRzRload->Real);
			RcalMag = sqrt(
					(float) pDftRcal->Real * pDftRcal->Real
							+ (float) pDftRcal->Image
									* pDftRcal->Image);
			RcalPhase = atan2(-pDftRcal->Image,
					pDftRcal->Real);
			RzMag = sqrt(
					(float) RzReal * RzReal
							+ (float) RzImage * RzImage);
			RzPhase = atan2(-RzImage, RzReal);
			RloadMag = sqrt(
					(float) pDftRload->Real
							* pDftRload->Real
							+ (float) pDftRload->Image
									* pDftRload->Image);
			RloadPhase = atan2(-pDftRload->Image,
					pDftRload->Real);

			RzMag = (AppIMPCfg.RcalVal * RcalMag * RzMag)
					/ (RzRloadMag * RloadMag);
			RzPhase = -(RcalPhase + RzPhase - RloadPhase
					- RzRloadPhase);
...
AD5940Err AppIMPISR(void *pBuff, uint32_t *pCount) {
	...
	*pCount = 0;
	...
	SLPKEY_LOCK); /* Prohibit AFE to enter sleep mode. */

	if (AD5940_INTCTestFlag(
	AFEINTC_0,
	AFEINTSRC_DATAFIFOTHRESH) == bTRUE) {
