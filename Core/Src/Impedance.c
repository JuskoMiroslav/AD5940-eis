...
/*
 ...
 It includes basic configuration for sequencer generator and application related
 parameters
 */
AppIMPCfg_Type AppIMPCfg = {
    ...
    .HstiaRtiaSel = HSTIADERLOAD_100R,
...
int32_t AppIMPDataProcess(int32_t *const pData, uint32_t *pDataCount) {
  ...
  *pDataCount = 0;
  ...
  DataCount = (DataCount / 6) *
              6; /* We expect Rz+Rload, Rload and RCAL data, . One DFT result
  ...
  for (uint32_t i = 0; i < ImpResCount; i++) {
    if (1) {
      ...
      RzRloadMag = sqrt((float)pDftRzRload->Real * pDftRzRload->Real +
                        (float)pDftRzRload->Image * pDftRzRload->Image);
      ...
      RloadMag = sqrt((float)pDftRload->Real * pDftRload->Real +
                      (float)pDftRload->Image * pDftRload->Image);
